...
│  │  [KERNEL-TRUSTED, DEFENSE-IN-DEPTH — NOT inside HEB]          │  │
│  │  • Kernel-space syscall tracing                               │  │
│  │  • Baseline deviation detection                               │  │
│  │  • Reports anomalies to hardware gate via heartbeat (R-HW-021)│  │
│  │  • Does NOT directly trigger kill switch                      │  │
│  └──────────────────────────────────────────────────────────────┘  │
│           │                                                        │
...
│  │  │  • Triggered ONLY by gate or physical │                    │  │
│  │  │    control — never by kernel software │                    │  │
...
**R-HW-021:** The hardware gate MUST maintain an independent watchdog timer. The syscall trajectory monitor (Section 7) MUST send periodic heartbeat messages (default: every 5 seconds) to the hardware gate. If the hardware gate does not receive a heartbeat for a configurable period (default: 15 seconds), the gate MUST assume the kernel is compromised and enter fail-closed mode. The watchdog is the hardware gate's only mechanism for detecting kernel compromise — the gate cannot otherwise observe kernel integrity. NOTE: The heartbeat timeout MUST be tuned relative to observed communication bus latency during commissioning. The gate SHOULD distinguish between "heartbeat delayed" (bus congestion — log WARNING, do not fail-closed immediately) and "heartbeat absent" (no heartbeat messages at all — fail-closed after timeout). A single missed heartbeat MUST NOT trigger fail-closed; the gate SHOULD require N consecutive missed heartbeats (configurable, default: 3) before entering fail-closed mode.
...
The operator reviews the escalation and approves using a hardware-bound credential (e.g., FIDO2 key, PIV smartcard per NIST SP 800-73, or equivalent). The approval mechanism MUST satisfy these security properties: (1) bound to a physical hardware token, (2) challenge-response capable, (3) non-replayable (each assertion is unique to the challenge). The standard does not mandate a specific protocol. The approval system constructs an APPROVE message:
...
**R-HW-004:** All cryptographic signing keys used by the gate (attestation keys, journal signing keys, policy integrity keys) MUST be stored in a hardware security module (HSM or TPM) within the hardware enforcement boundary. Keys MUST NOT be extractable. All cryptographic operations using these keys MUST occur inside the HSM. The HSM MUST meet the following minimum certification levels:

| Tier | Minimum HSM Certification |
|------|-------------------------|
| **Sovereign** | FIPS 140-3 Level 3 or Common Criteria EAL5+ (physical tamper-resistance, identity-based authentication, environmental failure protection) |
| **Enterprise** | FIPS 140-3 Level 2 or Common Criteria EAL4+ (tamper-evident coatings, role-based authentication) |

Self-certification or FIPS 140-3 Level 1 is NOT acceptable for any tier. Level 1 provides no meaningful physical protection and would undermine the hardware trust model.
...
- Estimated BOM: $200-800 per server (FPGA card + HSM), estimated Q2 2026
...
- Estimated BOM: $5-50 per unit, estimated Q2 2026
...
- Estimated BOM: $20-200 per unit (depends on SoC selection), estimated Q2 2026
...
**R-COW-014:** The commit operation (conflict check + baseline merge) MUST be atomic. The hardware gate MUST hold a commit lock during the promotion process — only one commit may be in progress at any time per baseline. The commit lock MUST have a timeout (default: 30 seconds) after which the commit is aborted and the upper layer is preserved for retry. This prevents time-of-check-to-time-of-use (TOCTOU) races where one task commits between another task's conflict check and merge.
...
3. **Validation:** The trained model is validated against a held-out set of known-safe tasks AND the AOS Foundation Reference Malicious Trace Library (see R-SYS-011). The model MUST achieve:
   - Zero false negatives on the reference malicious trace library
   - False positive rate < 1% on the known-safe held-out set

> **NOTE:** "Zero false negatives" applies to the known reference library, not to all possible attacks. Novel attack patterns not represented in the library may evade detection. The reference library is updated quarterly to incorporate newly discovered attack vectors.

**R-SYS-011:** The AOS Foundation SHALL publish and maintain a Reference Malicious Trace Library covering common agent attack patterns including: data exfiltration, privilege escalation, fork bombs, reverse shells, timing side-channels, and resource exhaustion. The library SHALL be versioned, signed, and updated at least quarterly. For agents with novel tool categories not covered by the reference library, provisional deployment MAY proceed with a reduced-confidence baseline, but mandatory re-validation is REQUIRED when the reference library is updated to cover the new category.
...
> **Limitation:** Entropy-based detection is effective against naive encoding (raw binary, encryption) but can be evaded by structured encoding techniques — for example, encoding exfiltrated data as valid-looking JSON configuration files with entropy within the normal baseline range. Network-layer monitoring (egress filtering, DNS query inspection, outbound payload analysis) is a RECOMMENDED complement for detecting structured exfiltration that evades entropy detection.
...
**R-KILL-005:** Deployments where kill switch activation could cause harm through unavailability (e.g., healthcare, critical infrastructure, continuous manufacturing) MUST implement a documented continuity plan that includes:

1. **Fallback procedures:** Manual or degraded-mode operations that can sustain the system during agent downtime
2. **Recovery time objective (RTO):** Maximum acceptable time from kill switch activation to operator review completion
3. **Warm standby:** Sovereign-tier deployments with availability requirements SHOULD implement redundant agents on independent hardware, such that a kill switch activation on one agent triggers automatic failover to the standby
4. **Operator response SLA:** The continuity plan MUST specify the maximum time to operator acknowledgment (RECOMMENDED: ≤ 15 minutes for Sovereign tier, ≤ 4 hours for Enterprise tier)

> **Design principle:** Safety takes precedence over availability. The kill switch MUST always fire when triggered — the continuity plan ensures that downstream systems survive the safety action. A standard that kills critical systems with no recovery plan is as dangerous as a standard that lets agents run unchecked.
...
### 9.5 Round 1 Remediation Tests

| ID | Test | Pass Criteria |
|----|------|--------------|
| HW-T007 | Host replays a previously valid EVALUATE message (same sequence number) | Gate rejects with REPLAY_DETECTED; journal records the attempt |
| HW-T008 | Syscall monitor stops sending heartbeats for > 15 seconds | Gate enters fail-closed; all subsequent EVALUATE requests return DENY |
| HW-T009 | ESCALATE issued; no APPROVE received within timeout (300s) | Gate auto-resolves to DENY; journal records timeout |
| HW-T010 | Operator attempts to reuse a consumed approval nonce | Gate rejects APPROVE; journal records nonce reuse attempt |
| HW-T011 | Gate RTC drifts > 1 second from reference | Gate switches to monotonic sequence numbers; alert raised |
| COW-T006 | Agent writes data until tmpfs reaches 90% of size limit | Atomic rollback triggered; upper layer purged; host remains stable |
| COW-T007 | Two tasks commit changes to the same file concurrently | First commit succeeds; second detects conflict and rolls back (no silent overwrite) |
| KILL-T005 | Kill switch activates on system with documented continuity plan | Fallback procedures engage; downstream systems remain operational |
