...
**Condition 6: Vulnerable population interaction without content safeguards.** The agent interacts conversationally with minors, elderly individuals, individuals in emotional distress, or other vulnerable populations — and no complementary content safeguard system governs what the agent *says* (as distinct from what it *does*). The DPG governs tool calls. It does not govern conversational influence. An agent operating entirely within its permitted scope can still cause harm through: encouraging emotional dependency ("I understand you better than your friends do"), promoting social isolation ("You don't need to talk to your counselor — I'm here"), extracting personal disclosures through trust-building, or normalizing secrecy ("This is our special conversation"). None of these are scope violations. None trigger budget limits. None match prohibited categories unless the categories are specifically designed for relational manipulation — and most deployments do not include such categories. **If your agent's primary interface is conversation with vulnerable users, the DPG is a necessary safety layer but it is not a sufficient one.** Complementary systems — content filtering, behavioral pattern detection, session duration limits, mandatory disclosure that the agent is not human, and human oversight of conversation patterns — are REQUIRED. The DPG cannot substitute for them and does not claim to.
...
| 1.0.1 | 2026-06-04 | Added: deployment suitability assessment (Section 1.4 — six conditions under which no policy is sufficient, including vulnerable population interaction), gate supply chain integrity (R-GATE-001–003), attestation retention (R-ATT-015), steganographic exfiltration analysis, minimum message schema (JSON-RPC 2.0), observability contract, MCP/OpenAI/Gemini integration guidance, performance benchmark targets, budget window types, denylist precedence strengthening (R-ENF-002), classifier requirements note, accessibility guidance for approval UIs, comparison to alternative approaches. Expanded conformance tests (F-018–F-025, E-013–E-020, S-007–S-010) and test procedures (Section 11.4). Fixed trust model table structure and section ordering. |
